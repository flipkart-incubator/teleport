...
func (oas *OIDCAuthService) validateOIDCAuthCallback(ctx context.Context, diagCtx *SSODiagContext, q url.Values) (*OIDCAuthResponse, error) {
	if errParam := q.Get("error"); errParam != "" {
		...
		return nil, trace.WithUserMessage(trace.OAuth2(oauth2.ErrorInvalidRequest, errParam, q), "OIDC provider returned error: %v [%v]", errDesc, errParam)
	}

	code := q.Get("code")
	if code == "" {
		return nil, trace.WithUserMessage(trace.OAuth2(
			oauth2.ErrorInvalidRequest, "code query param must be set", q), "Invalid parameters received from OIDC provider.")
	}

	stateToken := q.Get("state")
	if stateToken == "" {
		return nil, trace.WithUserMessage(trace.OAuth2(
			oauth2.ErrorInvalidRequest, "missing state query param", q), "Invalid parameters received from OIDC provider.")
	}
	...
	if len(connector.GetClaimsToRoles()) == 0 {
		return nil, trace.WithUserMessage(trace.BadParameter("no claims to roles mapping, check connector documentation"),
			"Claims-to-roles mapping is empty, SSO user will never have any roles.")
	}
